...
A Spec Kit preset for building **state-of-the-art, full-stack Next.js web applications** (including SaaS) with Spec-Driven Development.
...
Add the preset to a Spec Kit project, either from a local clone or directly from the repo:

```bash
# Local clone
git clone https://github.com/Satcomx00-x00/speckit-nextjs-SaaS.git
specify preset add --dev ./speckit-nextjs-SaaS

# Or, if your Spec Kit version supports remote presets
specify preset add github:Satcomx00-x00/speckit-nextjs-SaaS
```
...
| Command                          | What it does                                                                                                                                                                                                                                                     |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/speckit.feature <name>`        | Full-stack feature scaffold. Walks through all 10 phases: Zod schemas → Drizzle table → DAL → Services → Server Actions → RSC page → Client form → Inngest worker → Tests. Supports `--auth`, `--skip-queue`, `--skip-tests`, `--fields`, `--dry-run`, and more. |
...
# for each feature — choose one path:

## Quick path (scaffold entire feature at once):
3. /speckit.feature <name>           # full-stack scaffold: schemas → DB → DAL → actions → UI → tests

## Manual path (step-by-step control):
